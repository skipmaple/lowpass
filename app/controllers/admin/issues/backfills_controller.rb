...
class Admin::Issues::BackfillsController < Admin::BaseController
  def create
    ...
    if Issue.daily.exists?(period_key: key)
      redirect_back_or_to admin_issues_path, alert: "这一天已有期"
    elsif key > today
      redirect_back_or_to admin_issues_path, alert: "还没到这一天"
    elsif key == today
      ...
      redirect_back_or_to admin_issues_path, notice: "已补生成 #{key}，正在抓取"
    end
  end
