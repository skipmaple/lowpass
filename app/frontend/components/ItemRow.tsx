...
function Meta({ item, adapter, rank }: { item: Item; adapter: Adapter; rank: number }) {
  ...
  if (adapter === 'hacker_news') {
    ...
    // RSS 源：作者是源站给的拉丁内容，走 Newsreader；中文作者由字体回退接住
    parts.push(<span style={{ fontFamily: 'var(--font-latin)', fontSize: 'var(--fs-13)', fontWeight: 500, color: 'var(--ink2)' }}>{item.author}</span>)
  }
...
// 兴趣标签：墨色反白小块，22px 高，纸色文楷 12（PRD 6.3、D16）
  ...
  return (
    <article className="item-row">
      ...
      <div className="item-body">
        {/* R-8.4 原文外链在新标签页打开 */}
        <a className="t item-title" href={item.url} target="_blank" rel="noopener noreferrer">
          {item.title}
        </a>
