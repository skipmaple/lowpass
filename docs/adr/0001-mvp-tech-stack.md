...
**2026-09-11 产品负责人决定（PRD D23）**：T8 延后到 P2，随管理后台一起落地。供应商、模型名与月费用上限在 `/admin/settings` 配置，密钥只从环境变量读；接入层按可切换供应商设计，A 与 B 的取舍随后台配置决定，C 自托管不在 P2 范围。P0 不生成推荐理由，读者页已预留 `reason` 与 `interest_tag` 的渲染。实现计划在 P2 阶段编写，包含生成任务、后台配置页与重生成入口。2026-09-14 实现（P2-④）：接入层按 OpenAI 兼容协议实现，供应商由后台的地址与模型名决定，密钥 `MODEL_API_KEY` 只从环境读。
