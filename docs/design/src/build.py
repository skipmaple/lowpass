...
pages = {
    # 第七轮：纸报定稿方向，每源前 10，三栏一版
    'Main.dc.html': X.front(),
    # 第六轮：完整一期 67 条（存档，对照）
...
canvas = {
    ...
    'artboards': [
        {'file': 'Main.dc.html', 'x': 0, 'y': 0, 'w': 1440, 'h': 2500, 'title': '纸报 · 每源前 10 · 三栏一版（Main）', 'page': P1},
    ...
    'annotations': [
        {'id': 'x1', 'x': 1560, 'y': 0, 'w': 640, 'page': P1, 'text': '纸报 · 每源前 10\n产品负责人定：每个来源只展示前 10，少即是多，PRD 同步改为默认 10。于是三个来源并排三栏，一版读完，栏尾一个去源站看完整榜单的外链。没有预告框，没有编造的导语。每个元素对应一条真实字段：报耳的日期、发布时间与条目数；数据带里的来源数、条目数、抓取用时、昨日条数（来自抓取记录）；索引条里的三个来源与计数（滚动时吸顶，点击跳到该栏）；列表里的序号、标题、分数、评论、时间、语言、star、今日新增、作者、摘要。\n易读：正文 17px Newsreader 400，行高 1.35 以上；次级墨色 #55504B 在浅纸上对比度约 6:1；大写只用于拉丁栏目标题；标题整行可点，悬停下划线。\n结构：三栏并排，栏间细线；Hacker News 十条编号列表；GitHub 十条带简介与语言、star，今日新增用绿徽章；Hackaday 十条带摘要。'},
