...
def login():
    ...
    btn = btn.replace('height: 44px', 'height: 40px')
    body = (
...
def home():
    ...
    weekly = (
        ...
        + chapter_head('本周周刊', '第 36 周 · 8月31日 至 9月6日')
        ...
        + mono('9月4日 发布') + '</article>\n'
        + more_link('阅读本周周刊')
        + '</div>\n'
    )
    return document(topbar(active='') + header + grid + weekly, 1440, 1240)
...
def daily_mobile():
    header = (
        ...
        '<span>9月8日</span><span style="font-size: 16px; color: ' + INK64 + ';">星期二</span></h1>\n'
        '</div>\n'
        '<div style="display: flex; gap: 4px; padding: 8px 10px 16px 10px;">'
        + button('9月7日', 'ghost', icon_left='chevron-left', height=44) + button('归档', 'ghost', height=44)
        + button('9月9日', 'ghost', icon_right='chevron-right', height=44, disabled=True) + '</div>\n'
    )
...
def daily_archive():
    ...
    header = issue_header('日刊归档', '2026 年 9 月', '', controls).replace(mono('日刊归档'), label('日刊归档'))
    months = ''.join(
        '<span style="font-size: 15px; color: ' + (INK if m == '9月' else INK64) + '; font-weight: ' + ('500' if m == '9月' else '400') + ';">' + m + '</span>'
        for m in ['9月', '8月', '7月', '6月', '5月']
    )
    left = ('<div style="display: flex; flex-direction: column; gap: 12px;">' + label('2026') + months + '</div>\n')
    ...
    legend = ('<div style="display: flex; gap: 24px; padding-top: 24px;">'
              '<span style="display: inline-flex; align-items: center; gap: 8px;">' + dot_ok + label('已发布', INK64, 14) + '</span>'
              '<span style="display: inline-flex; align-items: center; gap: 8px;">' + dot_empty + label('空刊', INK64, 14) + '</span>'
              '<span style="display: inline-flex; align-items: center; gap: 8px;">' + dot_missing + label('未生成', INK64, 14) + '</span></div>')
