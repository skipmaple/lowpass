...
- 登录墙（D1、6.1）：`ApplicationController` 默认 `require_authentication`；放行只有 `sessions#new / create / failure` 与 `/up`；未登录 GET → `/login?next=<fullpath>`，非 GET → `/login`；`next` 只接受 `\A/(?![/\\])[^[:cntrl:]]*\z`（不含任何控制字符），其他当没有（R-5.7、AC-5.7）；所有带计算目标的 `redirect_to` 加 `allow_other_host: false`。
