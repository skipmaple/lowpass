...
- [ ] Add behavior regressions: menu arrow/Home/End navigation + Escape/return focus; Dialog initial cancel focus + restoration; reader items expose no reason-regeneration control; missing issue has a useful latest-readable action; weekly navigation points to latest readable content.
...
- [ ] Fix F01/F02/F06/F08: responsive masthead/source tabs throughout 320–1440px, 44px hit areas, lang=zh-CN, main/skip link/h1/title. Reader items do not expose reason-regeneration controls; the admin issue list retains guarded whole-issue regeneration and prerequisite explanations. All feedback must describe actual server outcome, not assume success.
